...
function PostQueue() {
  ...
  const handlerDelete = (id) => {
    axios
      ...
      .then((res) => {
        const updatedQueues = requestQueues.filter((item) => item.id !== id);
        setRequestQueues(updatedQueues);
      })
      .catch((err) => {});
  };
            ...
            <div className="col-md-12">
              ...
              {requestQueues.map((resQueue, index) => (
                <div className="post-entry-1 border-bottom" key={index}>
                  ...
                  <h2 className="mb-2">
                    <a href={resQueue.source_url} target="_blank" rel="noopener noreferrer">
                      {resQueue.source_url}
                    </a>
                  </h2>
                  <span className="author mb-3 d-block">
                    <button
                      onClick={() => handlerUpdate(resQueue)}
                      type="button"
                    ...
                    |{" "}
                    <button
                      onClick={() => handlerDelete(resQueue.id)}
                      type="button"
